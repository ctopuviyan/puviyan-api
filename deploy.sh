...
if command -v gcloud &> /dev/null; then
  ...
  gcloud run deploy puviyan-api-$ENVIRONMENT \
    ...
    --region asia-south1 \
    --project $PROJECT_ID \
    --set-env-vars NODE_ENV=$ENVIRONMENT,FIREBASE_PROJECT_ID=$FIREBASE_PROJECT_ID,PARTNER_FIREBASE_PROJECT_ID=$PARTNER_FIREBASE_PROJECT_ID,ORG_INVITE_CODE_SALT=$ORG_INVITE_CODE_SALT,PARTNER_PORTAL_URL=$PARTNER_PORTAL_URL,SMTP_HOST=$SMTP_HOST,SMTP_PORT=$SMTP_PORT,SMTP_SECURE=$SMTP_SECURE,SMTP_USER=$SMTP_USER,SMTP_PASSWORD=$SMTP_PASSWORD \
